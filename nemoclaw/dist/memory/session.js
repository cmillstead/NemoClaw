...
class SessionManager {
    ...
    start(model = null) {
        ...
        const id = (0, para_js_1.generateSessionId)();
    ...
    append(role, content) {
        ...
        const tokenCount = (0, compaction_js_1.estimateTokens)(content);
    ...
    runCompaction() {
        ...
        try {
            ...
            const result = (0, compaction_js_1.compact)(this.sessionId, messages, this.config.compactionThreshold);
            if (!result) {
            ...
            this.db.updateSessionStatus(this.sessionId, "active");
            this.logger.info(`Compaction complete: ${result.originalTokenCount} -> ${result.summaryTokenCount} tokens`);
            return true;
        }
        catch (err) {
    ...
    expandCompaction(compactionId) {
        ...
        if (!comp)
            return null;
        const messages = this.db.getMessagesInRange(this.sessionId, comp.message_range_start, comp.message_range_end);
        return messages
