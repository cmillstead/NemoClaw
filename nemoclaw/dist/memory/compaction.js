...
exports.extractFromMessages = extractFromMessages;
...
function extractRememberRequests(content) {
    const requests = [];
    const patterns = [
 ...
 */
function compact(sessionId, messages, threshold) {
    ...
    const compactionId = `comp-${new Date().toISOString().replace(/[-:T.Z]/g, "").slice(0, 14)}-${(0, node_crypto_1.randomBytes)(4).toString("hex")}`;
    const originalTokenCount = toCompact.reduce((sum, m) => sum + (m.token_count ?? estimateTokens(m.content)), 0);
    return {
        id: compactionId,
        summary,
...
function formatSummary(extraction) {
    const sections = [];
