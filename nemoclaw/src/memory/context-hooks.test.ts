  ...
  describe("handleSubagentEnded", () => {
    it("skips fact capture for non-completed subagents", () => {
      const ctx: SubagentEndedContext = {
        ...
        parentSessionId: sessionMgr.getSessionId()!,
        messages: [
          { id: 1, session_id: "sub-001", role: "user", content: "Remember this", token_count: 10, created_at: new Date().toISOString(), compacted: 0, compaction_id: null },
        ],
        exitReason: "timeout",
      };
    ...
    it("skips fact capture for internal maintenance subagents", () => {
      const ctx: SubagentEndedContext = {
        ...
        parentSessionId: sessionMgr.getSessionId()!,
        messages: [
          { id: 1, session_id: "sub-001", role: "user", content: "Remember this important fact", token_count: 10, created_at: new Date().toISOString(), compacted: 0, compaction_id: null },
        ],
    ...
    it("captures facts from completed subagent transcript", () => {
      const ctx: SubagentEndedContext = {
        ...
        parentSessionId: sessionMgr.getSessionId()!,
        messages: [
          { id: 1, session_id: "sub-001", role: "user", content: "Remember that the auth system uses JWT tokens", token_count: 20, created_at: new Date().toISOString(), compacted: 0, compaction_id: null },
          { id: 2, session_id: "sub-001", role: "assistant", content: "Noted.", token_count: 5, created_at: new Date().toISOString(), compacted: 0, compaction_id: null },
        ],
        exitReason: "completed",
      };
      const result = handleSubagentEnded(ctx, db, config, makeLogger());
